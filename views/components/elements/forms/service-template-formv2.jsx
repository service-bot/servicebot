...
import GenericInput from "../../utilities/input";
...
let selectList = (props) => {
    const {options} = props;
    return (<div>

        <Field name="default" component="select">
            {options.map(option => <option value={option.name}>{option.label}</option>)}
        </Field>
...
const selector = formValueSelector('syncValidation') // <-- same as form name
...
let SelectInput = (props) => {
    const {index, member, configValue} = props
    console.log("STUFF ! " ,configValue)
    return (<div>
        <TagsInput  {...props.service_template_properties[index].config.input} value={(configValue && configValue) || []}/>
        <SelectWidget configValue={props.configValue} {...props.service_template_properties[index].default}/>
    </div>)
}
...
const {index, typeValue, member, hasEmailValue, configValue, myValues, emailValue, change} = props;
    ...
    return (
        <div>
            ...
            <Field name={`${member}.inputType`} id="inputType" component="select">
                <option />
                <option value="text">Red</option>
                <option value="checkbox">Green</option>
                <option value="selectinput">Blue</option>
            </Field>

            {/*{typeValue && <Fields configValue={props.configValue} member={member} index={index} names={[member + ".default", member + ".config"]} component={CustomInputTypes[typeValue]}/>}*/}
            {typeValue && <RenderWidget member={member} configValue={props.configValue} widgetComponent={CustomInputTypes[typeValue].widget} configComponent={CustomInputTypes[typeValue].config}/>}
            <div>
                <label htmlFor="hasEmail">Has Email?</label>
                <div>
                    <Field onClick={change} name={`${member}.hasEmail`} id="hasEmail" component="input" type="checkbox"/>
                </div>
            </div>
            {hasEmailValue && <div>
                <label>Email</label>
                <div>
                    <Field name={`${member}.email`} component="input" type="email" placeholder="Email"/>
                </div>
            </div>}
...
CustomField = connect((state, ownProps) => {
    return {
        ...
        "typeValue" : selector(state, "references.service_template_properties")[ownProps.index].inputType,
   ...
   return {"change" : () => dispatch(change("syncValidation", `references.${ownProps.member}.email`, ""))}
})(CustomField)
    ...
    <ul>
        ...
        {fields.map((member, index) =>
            <li key={index}>
                ...
                <CustomField member={member} index={index}/>
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            <FormSection name="references">
                <FieldArray name="service_template_properties" component={renderCustom}/>
            </FormSection>
            <div>
...
ServiceForm = reduxForm({
    initialValues : {
        "username" : "hello",
      "references" : {
          "service_template_properties" : [{"email" : "booo", "hasEmail" : true}]
      }
    },
